            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (user.id !== currentUser.id) {
                    allUsersData.push({
                        ...
                        profilePicture: user.profilePicture || "../images/defaultUserLogo.png"
                    });
                }
            });
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (friendship.user1 === currentUser.id || friendship.user2 === currentUser.id) {
                    const friendId = friendship.user1 === currentUser.id ? friendship.user2 : friendship.user1;
                    ...
                    if (friend) {
                        friendsData.push({
                            ...
                            profilePicture: friend.profilePicture || "../images/defaultUserLogo.png",
...
async function loadFriendRequests() {
    try {
        ...
        if (requestsSnapshot.exists()) {
                    ...
                    if (request.senderID === currentUser.id) {
                        ...
                        if (receiver) {
                            sentRequestsData.push({
                                ...
                                receiverProfile: receiver.profilePicture || "../images/defaultUserLogo.png",
                        ...
                        if (sender) {
                            receivedRequestsData.push({
                                ...
                                senderProfile: sender.profilePicture || "../images/defaultUserLogo.png",
                                receiverID: request.receiverID,
                                timestamp: request.timestamp,
                                status: request.status
                            });
            ...
            receivedRequestsDiv.innerHTML = '<div class="loading">No received requests</div>';
        }
    } catch (error) {
        console.error("Error loading friend requests:", error);
        sentRequestsDiv.innerHTML = '<div class="error">Error loading sent requests</div>';
        receivedRequestsDiv.innerHTML = '<div class="error">Error loading received requests</div>';
    }
}

function renderUserList(users, container, type) {
    if (!users || users.length === 0) {
        container.innerHTML = `<div class="loading">No ${type === 'friends' ? 'friends' : 'users'} found</div>`;
        return;
    }
    ...
    users.forEach(user => {
        ...
        if (type === 'friends') {
            ...
            removeBtn.addEventListener('click', () => removeFriend(user.friendshipId));
...
function renderRequestList(requests, container, type) {
    ...
    requests.forEach(request => {
        ...
        if (type === 'received') {
            ...
            acceptBtn.addEventListener('click', () => acceptFriendRequest(request.id, request.senderID));
            declineBtn.addEventListener('click', () => declineFriendRequest(request.id));
        } else if (type === 'sent') {
            const cancelBtn = requestElement.querySelector('.cancel-btn');
            cancelBtn.addEventListener('click', () => declineFriendRequest(request.id));
        }
...
async function sendFriendRequest(user) {
    try {
        ...
        const existingRequest = sentRequestsData.find(req =>
            req.receiverID === user.id
        ) || receivedRequestsData.find(req =>
    ...
    if (userSearchInput) {
        userSearchInput.addEventListener('input', (e) => {
            ...
            if (activeTab === 'all-users') {
                ...
                const filteredRequests = sentRequestsData.filter(request =>
                    (request.receiverName && request.receiverName.toLowerCase().includes(searchTerm)) ||
                    (request.receiverEmail && request.receiverEmail.toLowerCase().includes(searchTerm))
                );
                renderRequestList(filteredRequests, sentRequestsDiv, 'sent');
            }
            else if (activeTab === 'received-requests') {
                const filteredRequests = receivedRequestsData.filter(request =>
                    (request.senderName && request.senderName.toLowerCase().includes(searchTerm)) ||
                    (request.senderEmail && request.senderEmail.toLowerCase().includes(searchTerm))
                );
