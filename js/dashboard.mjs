...
async function fetchUserStatus() {
    ...
    if (user) {
        ...
        // Construct the correct user reference using the email
        const userEmailKey = user.email.replace(".", ",");
        const userRef = ref(database, "users/" + userEmailKey);

        try {
            const [statusSnapshot, userSnapshot] = await Promise.all([get(userStatusRef), get(userRef)]);

            if (statusSnapshot.exists()) {
                const userStatus = statusSnapshot.val().state;
                document.getElementById("userStatus").textContent = userStatus;
            } else {
                document.getElementById("userStatus").textContent = "No status available";
            }
        } catch (error) {
